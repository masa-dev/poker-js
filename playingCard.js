...
function cpuMovement() {
    let i, j, k = 0, cnt = 0;
    let judge = opponentsHand.pokerJudgement();
    let oppExcNum = [];
    ...
    if(judge == 9) {
        ...
        for(i = 0; i < 2; i++) {
            for(j = 0; j < 5; j++) {
                if(opponentsHand.cards[i].suit == opponentsHand.cards[j].suit) {
                    cnt++;
                }
            }
            if(cnt == 4) {
                for(j = 0; j < 5; j++) {
                    if(opponentsHand.cards[i].suit != opponentsHand.cards[j].suit) {
                        opponentsHand.discardAnyCards(1, [j], discard);
                        opponentsHand.drawCard(deck, j);
                    }    
                }
            }
            cnt = 0;
        ...
        //フラッシュを狙えない時
        opponentsHand.discardAnyCards(5, [0,1,2,3,4], discard);
        for(i = 0; i < 5; i++) {
    ...
    //ワンペア～スリーカードの時
        ...
        for(i = 0; i < 5; i++) {
            for(j = 0; j < 5; j++) {
                if(opponentsHand.cards[i].number == opponentsHand.cards[j].number) {
                    cnt++;
                }
            }
            if(cnt == 1){
        ...
        opponentsHand.discardAnyCards(oppExcNum.length, oppExcNum, discard);
        for(j = 0; j < oppExcNum.length; j++) {
            opponentsHand.drawCard(deck, oppExcNum[j] - 1);
        }
    }
}
